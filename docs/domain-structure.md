# Domain Structure
To register a subdomain, you need to create a new JSON file in the `domains` directory through a pull request. For example, to register `example.aucn.me`, you would create a file named `example.json` in the `domains` directory. The full path would be `domains/example.json`.

## Filename

**Note**: You can include `.` (dots) in your filename to register a sub-subdomain (e.g., `blog.example.aucn.me`). However, each segment of your subdomain must meet the following criteria:
...
- `a.aucn.me.json` (filename contains `.aucn.me`)
...
```json
{
  "description": "Documentation website for aucn.me",
...
```
Note: Please refer to the [frequently asked questions](https://docs.aucn.me/faq/) for clarification on what or who we allow NS records for. If you want a example on what we want as the reasonings, you can [checkout this PR](https://github.com/is-a-dev/register/pull/16758).
